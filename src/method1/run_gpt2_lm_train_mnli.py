        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                for line_id, line in enumerate(f):
                    ...
                    label = line[0]
                    premise = line[5].strip()
                    hypothesis = line[6].strip()

                    if len(premise.split(' ')) <= 15:
                        self.corpus.append(premise)
                    if len(hypothesis.split(' ')) <= 15:
                        self.corpus.append(hypothesis)
...
def collate(examples):
    corrupted_sentences = [corrupt.corrupt(sentence, save_prob=.5, shuffle_prob=.2, replace_prob=.8) for sentence in examples]
...
def generate(model, tokenizer, encoded_prompt, num_return_sequences=30, repetition_penalty=1.):
    '''
    ...
    while True:
        output_sequences = model.generate(
            ...
            max_length=9 + len(encoded_prompt[0]),
            temperature=1.,
            top_k=40,
            top_p=1.,
            repetition_penalty=repetition_penalty,
        ...
        for text, generated_sequence in zip(texts, output_sequences):
            ...
            if '\n' in text:
                ...
                if text not in new_texts:
                    ...
                    new_output_sequences.append(generated_sequence)
        ...
        if len(new_output_sequences) > 50:
            break

    generated_sequences = []
    entire_texts = []

    for generated_sequence_idx, generated_sequence in enumerate(new_output_sequences):
...
def generate_file(args, model, tokenizer):
    ...
    for sentence in eval_dataset:
        ...
        tokenized_prompt = tokenizer.convert_tokens_to_ids(tokenizer.tokenize('%s %s' % (corrupt.corrupt(sentence, save_prob=1, shuffle_prob=0, replace_prob=0), sep_token)))
        with torch.no_grad():
        ...
        if random.random() > 0.95:
            pickle.dump(generated_sequences_list, open(args.eval_data_file + '.paraphrase.trainONmnli12000.%.2fpenalty.maxlength9.pkl' % (args.repetition_penalty), 'wb'))
