...
def generate(model, tokenizer, encoded_prompts, num_return_sequences=30, repetition_penalty=1.):
    '''
    ...
    for generation_round in range(100):
        ...
        output_sequences = model.generate(
            ...
            max_length=9 + len(encoded_prompt[0]) + int(generation_round / 10),
            temperature=1.,
            top_k=40,
            top_p=1.,
            repetition_penalty=repetition_penalty,
...
def generate_file(args, model, tokenizer):
    ...
    for sentence in eval_dataset:
        ...
        tokenized_prompts = [tokenizer.convert_tokens_to_ids(tokenizer.tokenize('%s %s' % (corrupt.corrupt(sentence, save_prob=1, shuffle_prob=0, replace_prob=0.3), sep_token))) for i in range(20)]
        with torch.no_grad():
            input_ids = [torch.tensor([tokenized_prompt]).long().to(args.device) for tokenized_prompt in tokenized_prompts]
            entire_texts, generated_sequences = generate(model, tokenizer, input_ids, num_return_sequences=200, repetition_penalty=args.repetition_penalty)
        ...
        if random.random() > 0.95:
            pickle.dump(generated_sequences_list, open(args.eval_data_file + '.trainONmnli12000.paraphrase.%.2fpenalty.maxlength9.pkl' % (args.repetition_penalty), 'wb'))
