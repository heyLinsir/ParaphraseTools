...
generation_file_postfix = 'trainONmnli12000.paraphrase.1.00penalty.maxlength9'
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        with open(file_path, encoding="utf-8") as f:
            for text in f:
                ...
                tokenized_paraphrases = [tokenizer.convert_tokens_to_ids(tokenizer.tokenize('%s %s %s' % (cls_token, paraphrase, end_token))) for paraphrase in paraphrases[0]]
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for batch, num_paraphrases, paraphrases in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            paraphrase_score_pairs = [(paraphrase, score) for paraphrase, score in zip(paraphrases, scores)]

            paraphrase_score_pairs = sorted(paraphrase_score_pairs, key=lambda x: x[1], reverse=True)
            score_list.append(np.mean([score for (paraphrase, score) in paraphrase_score_pairs[:10]]))
